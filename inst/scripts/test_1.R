...
omic_collection <- OmicSignature::OmicSignatureCollection$new(
  OmicSigList = base::list(omic_signature_1, omic_signature_MDA_CYP),
  metadata = metadata
)
...
SigRepo::addCollection(
  ...
  visibility = FALSE,                 # Whether to make the collection public or private
...
metadata <- base::list(
  # required attributes:
  signature_name = "Myc_reduce_mice_liver_24m_v4",
  organism = "Mus Musculus",
  direction_type = "bi-directional",
  assay_type = "transcriptomics",
  phenotype = "Myc_reduce",
...
saveRDS(omic_signature, base::file.path(signature_path, "omic_signature_4.RDS"))
