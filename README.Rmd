...
This readme will strictly cover the functions of SigRepo, OmicSignature documentation can be found at the top of the page.



```{r}
# Getting the signature path
signature_path <- base::system.file("inst/extdata/signatures", package = "SigRepo")
...
print(omic_signature_1)
print(omic_signature_2)
...
## **Example 1**: Upload `omic_signature_1` signature
...
## **Example 2**: Upload `omic_signature_2` signature
...
if(nrow(signature_tbl) > 0){
  SigRepo::deleteSignature(
    conn_handler = conn_handler, 
    signature_name = "test_sig_2" # picking an id from the signature tbl output
  )
}
```
