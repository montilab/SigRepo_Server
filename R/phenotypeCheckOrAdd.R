...
#' @export
phenotypeCheckOrAdd <- function(phenotype = NULL, thisHandle = newConnHandle(),
                                verbose = T) {
  ...
  if (!is.null(phenotype) && typeof(phenotype) == "character") {
    ...
    phenotypeIdInsert <- sqlFindingQuery("phenotypes",
      c("phenotype_id"),
      ins = list("phenotype" = phenotype)
    )$phenotype_id[1]
    if (is.na(phenotypeIdInsert)) {
      if (verbose == T) {
        print(paste("This phenotype [", phenotype, "] does not exist in our DB. adding now."))
      }
      insertPhenotypeQuery <- paste("INSERT INTO phenotypes(phenotype) values(", singleQuote(phenotype), ");")
  ...
  else if (!is.null(phenotype) && typeof(phenotype) != "character") {
    if (verbose == T) {
