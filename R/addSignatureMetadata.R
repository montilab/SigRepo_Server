...
#' @export
addSignatureMetadata <- function(sig_name,
                         species_name,
                         platform_name,
                         cell_line,
                         phenotype = NULL,
  ...
  thisSubmitterId <-
    sqlFindingQuery("submitters", c("submitter_id"),
                    ins = list("submitter_name" = c(uploader)))$submitter_id
  now <- Sys.time()
  signature_name <- singleQuote(sig_name)
  my_species <- (species_name)
  species_id_insert <- sqlFindingQuery("species", c("species_id"),
                                       ins = list("species" = my_species))$species_id[1]
