...
addRefFeatureSet <- function(
  ...
  if(assay_type == "transcriptomics"){
    SigRepo::addTranscriptomicsFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }else if(assay_type == "proteomics"){
    SigRepo::addProteomicsFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }else if(assay_type == "metabolomics"){
    SigRepo::addMetabolomicsFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }else if(assay_type == "methylomics"){
    SigRepo::addMethylomicsFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }else if(assay_type == "genetic_variations"){
    SigRepo::addGeneticVariationsFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }else if(assay_type == "dna_binding_sites"){
    SigRepo::addDNABindingSitesFeatureSet(conn_handler = conn_handler, feature_set = feature_set)
  }
