...
replaceDifexpCol <- function(colname) {
	...
	colname <- colname %>%
		...
		dplyr::recode(
			...
			"adj.p.val" = "fdr", "adj.p.value" = "fdr", "adj.p" = "fdr",
			"q.value" = "q_value", "qval" = "q_value"
		)
	colnameMissing<-setdiff(c("probe_id", "symbol", "score", "p_value", "fdr"), colname)
	if (length(colnameMissing)>0){
		warning("Required column for OmicSignature object difexp: ", paste(colnameMissing, collapse=", "), 
