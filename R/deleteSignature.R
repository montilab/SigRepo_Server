...
deleteSignature <-
  function(signatureName,
    ...
    if (tolower(confirm) == "y") {
      sid <- as.integer(sqlFindingQuery(
        "signatures",
        c("signature_id"),
        ins = list("signature_name" = signatureName)
      )$signature_id[1])
      if (!sid) {
        stop(paste0("No signature exists under the name: ", signatureName))
      }
      else{
        # Remove from keywords
        sqlGeneric(
          paste0(
            "Delete from keyword_signatures where signature_id=",
            as.character(sid),
            ";"
          ),
          conn = connHandle
        ...
        sqlGeneric(
          paste0(
            "Delete from signature_to_collection where signature_id=",
            as.character(sid),
            ";"
          ),
          conn = connHandle
        ...
        sqlGeneric(
          paste0(
            "Delete from feature_signatures where signature_id=",
            as.character(sid),
            ";"
          ),
          conn = connHandle
        ...
        sqlGeneric(paste0(
          "Delete from signatures where signature_id=",
          as.character(sid),
          ";"
        ),
        conn = connHandle)
