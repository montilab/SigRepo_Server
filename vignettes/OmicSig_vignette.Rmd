...
For example, extract all features with a absolute score > 20 and fdr < 0.005.  
```{r}
OmicCol$extract.signature("abs(score) > 20; fdr < 0.005", bind = TRUE)
```

If `bind` is set to be `FALSE`, the output of each *OmicSignature* objects are 
provided individually as one element in a list.
```{r}
OmicCol$extract.signature("abs(score) > 20; fdr < 0.005", bind = FALSE)
```
