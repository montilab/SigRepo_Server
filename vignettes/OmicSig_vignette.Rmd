...
In this example, we previously specified the `score_cutoff` and `fdr_cutoff` 
when creating the metadata. We can use them here.
```{r}
temp_upsig <- cbind(
  filter(difexp, score > metadata$score_cutoff & adj_p < metadata$fdr_cutoff) %>% pull(symbol),
  filter(difexp, score > metadata$score_cutoff & adj_p < metadata$fdr_cutoff) %>% pull(score),
  "+"
)
head(temp_upsig)

temp_dnsig <- cbind(
  dplyr::filter(difexp, score < (-1) * metadata$score_cutoff & adj_p < metadata$fdr_cutoff) %>% pull(symbol),
  dplyr::filter(difexp, score < (-1) * metadata$score_cutoff & adj_p < metadata$fdr_cutoff) %>% pull(score),
  "-"
)
head(temp_dnsig)
```
...
```{r}
OmicObj$extract.signature("abs(score) > 20; adj_p < 0.001")
```
...
For example, extract all features with a absolute score > 20 and adj_p < 0.001.  
```
OmicCol$extract.signature("abs(score) > 20 & adj_p < 0.001", bind = TRUE)
```

If `bind` is set to be `FALSE`, the output of each *OmicSignature* objects are 
provided individually as one element in a list.
```{r}
OmicCol$extract.signature("abs(score) > 20 & adj_p < 0.001", bind = FALSE)
```
