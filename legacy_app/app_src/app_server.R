server <- function(input, output, session) {
  ...
  user_conn_handler_rv <- shiny::reactiveVal()
  ...
  shiny::observeEvent({
    ...
    user_tbl <- base::tryCatch({
      validateUser(conn_handler = user_conn)
    }, error = function(e) {
      base::print(e, "\n")
    ...
    if (base::nrow(user_tbl) == 0) {
      ...
      user_conn_handler_rv(user_conn)
  ...
  shiny::observeEvent({
    ...
    req(user_conn_handler_rv(), user_login_info())
    ...
    user_conn <- shiny::isolate({
      user_conn_handler_rv()
    })
    ...
    # Update handler
    user_conn$password <- user_password
  ...
  signature_db <- reactive({
    ...
    req(user_conn_handler_rv())
    signature_trigger()
    
    
    tryCatch({
      df <- SigRepo::searchSignature(conn_handler = user_conn_handler_rv())
  ...
  collection_db <- reactive({
    ...
    tryCatch({
      df <- SigRepo::searchCollection(conn_handler = user_conn_handler_rv())
