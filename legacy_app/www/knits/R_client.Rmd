...
The `SigRepo` package includes a suite of functions for easily storing and managing biological signatures and its constituents. Currently, `Sigrepo` is capable of storing, searching, and retrieving signatures and its signature collections from a MySQL Database. See <a href="">here</a> for how set-up the MySQL database with the appropriate schema.

In order to interact with a suite of functions in `SigRepo` package, the input data must represent an R6 object for the representation of signatures and signature collections, and they can be created using our in-house package, <a href="https://github.com/montilab/OmicSignature">OmicSignature</a>.
...
# Install SigRepo
devtools::install_github(repo = 'montilab/SigRepo', build = TRUE, force = TRUE)
...
conn_handler <- SigRepo::newConnHandler(
  ...
  host = "142.93.67.157", 
  port = 3306, 
  user = "montilab",  # placeholder, use your account user_name and password.
  password = "sigrepo"
)
```
...
signature_path <- base::system.file("extdata/signatures", package = "SigRepo")

# Read in the signature object
omic_signature_1 <- base::readRDS(base::file.path(signature_path, "omic_signature_1_revised.RDS"))
omic_signature_2 <- base::readRDS(base::file.path(signature_path, "omic_signature_2_revised.RDS"))
```
...
metadata <- base::list(
  # required attributes:
  signature_name = "Myc_reduce_mice_liver_24_readme",
...
missing_features <- SigRepo::addSignature(
  conn_handler = conn_handler, 
  omic_signature = omic_signature_2,
...
```{r}
signature_list <- SigRepo::getSignature(conn_handler = conn_handler, signature_name = "Myc_reduce_mice_liver_24m_v3")
```
...
if(nrow(signature_tbl) > 0){
  SigRepo::deleteSignature(
    conn_handler = conn_handler, 
    signature_id = "150" # picking an id from the signature tbl output
  )
}
```
...
if(nrow(signature_tbl) > 0){
  SigRepo::updateSignature(
    conn_handler = conn_handler, 
    signature_id = "257", # the id we want to update
