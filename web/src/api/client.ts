...
// The skill-runner / assistant service. In production this is a same-origin
// path (e.g. "/agent") that the host nginx proxies to the internal
// sigrepo-agent container, injecting the X-Agent-Token header so the browser
// never holds that shared secret. Override with VITE_AGENT_URL in dev.
...
// ---------- Website assistant (SSE) ----------

export interface ChatEvent {
  type: string;
  // AssistantMessage carries content blocks; result/error carry their own fields.
...
// Stream a turn of the admin assistant. Posts the prompt + the signed-in user's
// api_key to the agent's /chat endpoint and parses the Server-Sent Events,
// invoking onEvent for each. Resolves when the stream ends ("done").
export async function chatStream(
  prompt: string,
  opts: { sessionId?: string | null; onEvent: (e: ChatEvent) => void; signal?: AbortSignal }
): Promise<void> {
  ...
  if (!res.ok || !res.body) {
    const data = await res.json().catch(() => null);
    throw new ApiError(extractMessage(data) ?? `Assistant request failed (${res.status})`, res.status);
  }
  ...
  // SSE frames are newline-delimited "data: {json}" lines. Buffer across chunks
  // and emit each complete data line as it arrives.
  for (;;) {
