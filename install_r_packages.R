# Install BiocManager if not exists
if (!require("BiocManager", quietly = TRUE))
  utils::install.packages("BiocManager")

# Create a list of start-up packages 
startup_packages <- c("devtools", "dplyr", "rvest", "xml2", "yaml")
...
for(pkg in install_startup_packages) utils::install.packages(pkg)
...
bioconductor_pkgs <- required_pkgs[base::which(required_pkgs %in% biocPackages$Package)]
